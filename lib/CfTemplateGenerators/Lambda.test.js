...
describe('Lambda', () => {
  ...
  describe('.replaceEventInvokeConfigQualifierWithAlias', () => {
    const eventInvokeConfig = {
      ...
      Properties: {
        ...
        DestinationConfig: {
          OnFailure: {
            Destination: { 'Fn::GetAtt': ['Queue', 'QueueArn'] }
          }
        },
        Qualifier: '$LATEST',
    ...
    it('replaces the destination event invoke config \'s qualifier for an alias', () => {
      const functionAlias = 'TheFunctionAlias'
      const functionAliasName = 'Live'
      let expected = _.set('Properties.Qualifier', functionAliasName, eventInvokeConfig)
      expected = _.set('DependsOn', 'TheFunctionAlias', expected)
      const actual = Lambda.replaceEventInvokeConfigQualifierWithAlias(eventInvokeConfig, functionAlias, functionAliasName)
      expect(actual).to.deep.equal(expected)
    })
  })
})
