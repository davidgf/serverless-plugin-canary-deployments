...
describe('Lambda', () => {
  describe('.buildAlias', () => {
    ...
    it('should generate a AWS::Lambda::Alias resource', () => {
      const expected = baseAlias
      const actual = Lambda.buildAlias({
        alias,
        functionName,
        functionVersion
      })
          ...
          UpdatePolicy: {
            CodeDeployLambdaAliasUpdate: {
              ...
              BeforeAllowTrafficHook: {
                Ref: trafficShiftingSettings.beforeHook
              },
              DeploymentGroupName: {
                Ref: trafficShiftingSettings.deploymentGroup
              }
            }
          }
        }
  ...
  describe('.replacePermissionFunctionWithAlias', () => {
    ...
    it("replaces the permission's function for an alias", () => {
      ...
      const expected = _.set(
        ...
        permissionFunctionWithAlias,
        lambdaPermission
      )
      const actual = Lambda.replacePermissionFunctionWithAlias(
        lambdaPermission,
        functionAlias
      )
      expect(actual).to.deep.equal(expected)
  ...
  describe('.replaceEventMappingFunctionWithAlias', () => {
    ...
    it("replaces the event source mapping's function for an alias", () => {
      ...
      const expected = _.set(
        'Properties.FunctionName',
        eventMappingFunctionWithAlias,
        eventSourceMapping
      )
      const actual = Lambda.replaceEventMappingFunctionWithAlias(
        eventSourceMapping,
        functionAlias
      )
      expect(actual).to.deep.equal(expected)
